...
use kleio::attribution::{Attribute, Provenance, SourceRef, Tag};
use kleio::event_adapter::event_pack_from_genealogy_index;
use kleio::genealogy_event::{GenealogyEvent, GenealogyEventKind};
use kleio::model::{
    CalendarModel, DatePrecision, DateRange, DateValue, EventId, Family, FamilyId, GenealogyIndex,
    HistoricalDate, Name, Note as KleioNote, NoteId, Person, PersonId, Place, PlaceId, Sex,
};
use kleio::pack::{EventPack, PackId, PackKind, PackMetadata};
...
pub struct GedcomImportResult {
    /// Canonical event-pack projection for timeline/event consumers.
    pub event_pack: EventPack,

    /// Genealogy-specific projection retained for tree/archive compatibility.
    pub index: GenealogyIndex,

    pub summary: GedcomImportSummary,
}

/// Parse GEDCOM text and import it into a Kleio [`GenealogyIndex`](kleio::GenealogyIndex).
...
/// Map parsed `ged_io` data into a canonical event pack plus a genealogy projection.
#[must_use]
pub fn import_gedcom_data(data: &ged_io::types::GedcomData) -> GedcomImportResult {
    ...
    let event_pack =
        event_pack_from_genealogy_index(&index, gedcom_pack_metadata(data), PackKind::Genealogy);
    GedcomImportResult {
        event_pack,
        index,
        summary,
...
fn gedcom_pack_metadata(data: &ged_io::types::GedcomData) -> PackMetadata {
    let mut metadata = PackMetadata::new(PackId::new("import:gedcom"), "GEDCOM import");
    metadata.description = Some(format!(
        "Canonical event projection from GEDCOM import ({} individual(s), {} family record(s)).",
...
impl<'a> GedcomImporter<'a> {
    ...
    fn import_attribute_detail(
        ...
        self.events.push(GenealogyEvent {
            id: event_id,
...
fn map_event_kind(
    event: &ged_io::types::event::Event,
    event_type: Option<&str>,
) -> GenealogyEventKind {
    match event {
        ged_io::types::event::Event::Birth => GenealogyEventKind::Birth,
        ged_io::types::event::Event::Death => GenealogyEventKind::Death,
        ged_io::types::event::Event::Marriage => GenealogyEventKind::Marriage,
        ...
        ged_io::types::event::Event::Residence => GenealogyEventKind::Residence,
        ged_io::types::event::Event::Event => GenealogyEventKind::Other(
            event_type
                .filter(|value| !value.trim().is_empty())
                .unwrap_or("event")
                .to_string(),
...
#[cfg(test)]
mod tests {
    use kleio::{GenealogyEventKind, archive_genealogy_archive, deserialize_genealogy_archive};
    ...
    fn parses_and_imports_basic_gedcom() {
        ...
        let birth = result
            ...
            .events
            .iter()
            .find(|event| event.kind == GenealogyEventKind::Birth)
        ...
        let occupation = result
            ...
            .events
            .iter()
            .find(|event| event.kind == GenealogyEventKind::Occupation)
    ...
    fn archive_round_trip_rebuilds_runtime_index() {
        let result = import_gedcom_text(SAMPLE).expect("sample GEDCOM imports");
        let bytes =
            archive_genealogy_archive(&result.index.to_archive()).expect("archive serializes");
        let archive = deserialize_genealogy_archive(&bytes).expect("archive deserializes");
